...
//     document.getElementById('product-list').innerHTML=htmls
// })

  
const fetchApi=async (api)=>{
    const reponse=await fetch(api);//đoạn lấy dữ liệu mất mấy s đó nên phải dùng await để chờ promise đó thực thi và trả về kết quả
    const data=await reponse.json();//đoạn đổi sang js mất mấy s đó nên phải dùng await để chờ promise đó thực thi và trả về kết quả
    // ko dùng await thì nó chưa thực thi xong thì code dưới nó sẽ thực thi 
    console.log(data);
}
